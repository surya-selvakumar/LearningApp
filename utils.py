...
def generate_distractors(answer, count, model):
...
def addDistractors(qaPairs, count, model):
    if not model:
        print("Glove embeddings not found. Please download and place them in the following path: " + glove_file)
    
    for qaPair in qaPairs:
        distractors = generate_distractors(qaPair['answer'], count, model)
        qaPair['distractors'] = distractors
    
    return qaPairs


def generateQuestions(text, count, model):
    ...
    questions = addDistractors(orderedQaPairs[:count], 4, model)
    return questions
...
# print(generateQuestions(text, 10, model))